...
class MultiMotorControlGUI:
    ...
    def __init__(self, root):
        ...
        """
        self.root = root
        self.root.title("L30_标准版 V1.0.4")
    ...
    def setup_top_control(self):
        ...
        # 新增：手型显示标签
        self.hand_type_label = ttk.Label(port_frame, text="手型: 未知", style='Chinese.TLabel')
        self.hand_type_label.pack(side=tk.LEFT, padx=10)
    ...
    def setup_motor_control(self):
        ...
        for joint_name in columns:
            ...
            # 对于手腕电机，使用相同的角度范围
            if motor_id in [17, 117]:
                min_angle, max_angle = -50, 50  # 手腕的角度范围
            else:
    ...
    def connect_motor(self):
        ...
        success, message, hand_type = self.hand_api.connect_motor(selected_port)
        # 新增：连接后显示手型
        hand_type_str = {"right": "右手", "left": "左手"}.get(hand_type, "未知")
        self.hand_type_label.configure(text=f"手型: {hand_type_str}")
...
注意：校准时请保持手指位置不变"""
    ...
    def read_current_positions(self):
        ...
        for motor_id, motor_angle in positions.items():
            if motor_id in self.motors:
                ...
                print(f"开机读取电机 {motor_id} - 电机角度: {motor_angle:.1f}°, 关节角度: {joint_angle:.1f}°")
    ...
    def on_slider_change(self, motor_id, value):
        ...
        try:
            # 如果是手腕电机，获取当前正确的手腕电机ID
            original_motor_id = motor_id  # 保存原始motor_id用于访问self.motors
            if motor_id in [17, 117]:
                from l30_hand_api.motor_mapping import get_wrist_motor_id
                actual_motor_id = get_wrist_motor_id()
                if actual_motor_id != motor_id:
                    print(f"手腕电机ID不匹配，使用正确ID: {actual_motor_id}")
                    motor_id = actual_motor_id
            ...
            # 更新输入框显示（显示关节角度）- 使用original_motor_id
            self.motors[original_motor_id]['entry'].delete(0, tk.END)
            self.motors[original_motor_id]['entry'].insert(0, f"{limited_joint_angle:.1f}")
    ...
    def on_entry_change(self, motor_id):
        ...
        try:
            ...
            value = self.motors[original_motor_id]['entry'].get().strip()
            ...
            # 更新滑块 - 使用original_motor_id
            self.motors[original_motor_id]['slider'].set(limited_joint_angle)
    ...
    def run_all_sequences(self, interval, loop_count, selected_items):
        ...
        try:
            ...
            # 创建并启动动作执行线程
            self.action_thread = threading.Thread(target=self._run_sequences,
                                                  args=(interval, loop_count, selected_items))
    ...
    def _run_sequences(self, interval, loop_count, selected_items):
        ...
        try:
            ...
            for current_loop in range(loop_count):
                ...
                # 更新当前循环次数显示
                self.root.after(0, lambda c=current_loop + 1: self.loop_count_label.configure(
                    text=f"循环次数: {loop_count}/当前循环: {c}"))
