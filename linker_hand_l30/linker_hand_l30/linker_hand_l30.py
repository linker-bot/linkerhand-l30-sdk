...
class LinkerHandL30(Node):
    def __init__(self):
        ...
        if self.hand_type == "right":
            self.hand_api = DexterousHandController(device_id=0x01)
        elif self.hand_type == "left":
            self.hand_api = DexterousHandController(device_id=0x02)
        time.sleep(0.1)
        ...
        # 创建并启动线程
        ColorMsg(msg=f"SDK VERSION:{self.sdk_versoin}", color="green")
    ...
    def run(self):
        ...
        while True:
            ...
            time.sleep(0.005)
            state_arc, state_range = self.hand_api.get_current_state()
            # 使用映射关系初始化数组
            tmp_state_r = [0.0] * 17
